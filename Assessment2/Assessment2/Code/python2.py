...
def one(input):
	return ''.join([c+c+c for c in input])
...
def three(a):
    string_i = str(a)
    string_ii = str(a) + str(a)
    string_iii = str(a) + str(a) + str(a)
    string_iiii = str(a) + str(a) + str(a) + str(a)
...
def four(input1, input2):
	x = ''.join(''.join(x) for x in zip(input1,input2))
...
def five():
	randomlist =[random.randint(50,100) for i in range(5)]
	newlist = [i * 2 for i in randomlist]
	return newlist
...
def seven(a, b, c):
	emplist = [a]
	appb = emplist.append(b)
	appc = appb.append(c)
	s_list = appc.sort()
	for x in range(s_list):
		if s_list[2] - s_list[1] == s_list[1] - s_list[0]:
			return True
	return False
